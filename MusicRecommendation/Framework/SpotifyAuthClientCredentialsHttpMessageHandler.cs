...
namespace MusicRecommendation.Framework
{
    public class SpotifyAuthClientCredentialsHttpMessageHandler : DelegatingHandler
    {
        ...
        private async Task<string> GetAuthenticationTokenAsync()
        {
            ...
            var token = cache.Get(cacheKey) as string;
            ...
            if (token == null)
            {
                ...
                cache.Set(cacheKey, token, new DateTimeOffset(expireTime)); 
            }
            return token;
        }

        private async Task<SpotifyAuthenticationResponse> GetAuthenticationTokenResponse()
        {
            ...
            var authenticationResponse = JsonConvert.DeserializeObject<SpotifyAuthenticationResponse>(responseString);
